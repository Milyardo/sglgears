...
@extern object GLUT {
  ...
  //Need to fix scala-native #36
  //final def glutDisplayFunc(func: Function0Ptr[Unit]): Unit = extern
...
@extern object GL {
  ...
  final def glLightfv(light: GLEnum, pname: GLEnum, params: Array[GLFloat]): Unit = extern
  final def glGenLists(range: GLSizeI): GLUInt = extern
  final def glNewList(list: GLUInt, mode: GLEnum): Unit = extern
  final def glMaterialfv(face: GLEnum, pname: GLEnum, params: Array[GLFloat]): Unit = extern
